...
function App() {
    ...
    const [selectedId, setSelectedId] = useState(""); // 사용자가 select에서 선택한 값을 저장할 state

    const selectedCoin = coins.find(value => {
        return value.id === selectedId;
    });
    /*
    select에서 사용자가 뭔가를 선택한다면,
    선택된 option에 써놓은 value 속성의 값이 select의 선택 값(value 속성)을 변경하게 되고,
    select의 onChange가 발동되고,
    onChange에서 발동되는 기능은, 그 값을 selectedId에 덮어쓰고,
    selectedCoin하는 일이, coins에서 selectedId 값으로 검색해서 결과를 도출함
     */

    useEffect(() => {
        // 외부에서 데이터를 받아와서 coins에 저장하는 일
        // fetch() : 외부에서 데이터를 받아오는 비동기함수. 비동기함수는 .then()과 .catch()로 처리해야 함
        // .then(함수) : fetch()가 완료되면 실행되는 함수를 적어줘야 함      => then()은 메소드 체인으로 여러 개를 적어줄 수 있음
    ...
    return (
        <div>
            ...
            {loading ? (
                ...
                <div>
                    <select
                        value={selectedId}  // 우리가 value를 적어주게 되면서, 이에 대한 제어를 우리가 하게 됨 => Controlled Component
                        /* 원래 input이나 select나, 사용자가 입력한 값(또는 선택한 값)이 웹브라우저 상 value에 들어가게 되어있음.
                        => Uncontrolled Component => 우리(프로그래머 또는 이 프로그램)가 제어하지 않는 컴포넌트 */
                        onChange={event => {
                            // 사용자가 변경한 값을 저장해야 함
                    ...
                    {selectedCoin && (
                        <div
                            ...
                            <ul
                                ...
                                <li>
                                    {/* number 타입에 쓸 수 있는 메소드 .toLocaleString() : 숫자에 식별 콤마를 넣어 문자열로 변환 */}
